...
app = FastAPI()
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))   # repo root (package parent)
try:
    from chessdq import lane as _lane
...
def api_baseline(req: RunReq):
    out = run_script(["-m", "chessdq.chess_rl", str(req.n), "200"], timeout=1800)
    out["rows"] = load_rows()
    return out
...
def api_measure(req: RunReq):
    out = run_script(["-m", "chessdq.measure_elo", str(req.n)], timeout=1800)
    out["rows"] = load_rows()
    return out
...
def api_train_start(cfg: TrainReq):
    ...
    env = dict(os.environ,
               ...
               **({"QLEARN_DEV": cfg.device} if cfg.device else {}),
               ...
               QLEARN_TAG="final")
    script = "chessdq.ac_learn" if cfg.algo == "ac" else "chessdq.qlearn"
    ...
    TRAIN_PROC = subprocess.Popen([PY, "-m", script, str(cfg.epoch_games), str(cfg.max_epochs)],
    ...
    if cfg.post_rung_depth > 0 and cfg.algo != "ac":
        ...
        def _run_rung(best_ckpt, depth, games, tag):
            with open(os.path.join(ROOT, "data", "post_rung.log"), "a") as lg:
                subprocess.run([PY, "-m", "chessdq.claims_rung", best_ckpt, str(depth), str(games), tag],
...
<style>
  ...
  <div class="card"><h2>Settings (load the tuned config, edit, train)</h2>
    <div class="form">
      ...
      <div><label>native module</label><input id="rsearch_mod" type="text" value="rsearch4"></div>
