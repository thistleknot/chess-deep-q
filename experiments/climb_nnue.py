...
def main():
    ...
    for c in range(1, cycles + 1):
        ...
        run(["experiments/gen_labels.py", gen_s, "8", "10", CORPUS], f"climb_gen_{c}.log")
        recs = records()
        print(f"[climb {c}] train on {recs} records ...", flush=True)
        run(["experiments/train_nnue.py", epochs, "1024"], f"climb_train_{c}.log")
        print(f"[climb {c}] kill-check vs pst@d2 ...", flush=True)
        out = run(["experiments/measure_nnue.py", kc, "2"], f"climb_kc_{c}.log")
        ...
        subprocess.run([PY, "experiments/plot_nnue_trend.py"])
