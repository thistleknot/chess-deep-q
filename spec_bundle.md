...
- :Linear-value: is V(s) = tanh(wᵀφ(s)), where φ is the hand feature vector (`gbdt_features.features`: material + PST + castling, White-absolute) and w the LEARNED weights. The hand heuristic `pst` is itself a FIXED wᵀφ (hand-set weights); :Linear-value: LEARNS w by gradient descent — RL-tuning the heuristic's own weights rather than hand-setting them. It is µs-cheap (a dot product), CPU, and — unlike the conv net — carries the convergence guarantees of LINEAR function approximation (on-policy linear TD converges; Sutton & Barto). The alpha-beta leaf eval is wᵀφ(s) (pre-tanh; monotone, so move choice is unchanged); tanh only bounds the TRAINING target to the outcome range.
- :Afterstate-target: is the RL signal: Q(s,a) = wᵀφ(s·a) (:Afterstate-action-value:), and w is regressed toward the self-play OUTCOME z / :Lambda-return: (White-absolute), SWAPPING the supervised SF-cp label (`train_linear`/`train_gbdt`) for the committed outcome. Update = gradient descent on MSE(tanh(wᵀφ(s)), target): w ← w − α·(tanh(wᵀφ) − target)·(1 − tanh²)·φ (MC with target z; semi-gradient TD(λ) is the bootstrap extension). This is exactly TD-Gammon — shallow linear FA + TD(λ) on self-play outcomes.
