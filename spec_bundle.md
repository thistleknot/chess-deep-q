...
- :Loss-trace: — the loop appends per-update ACTOR loss and CRITIC loss (TWO curves) to a persistent trend log and plots them over training time. Because actor and critic are adversarial (ch5 fig 5.13), the combined loss is uninformative/chaotic BY DESIGN — the two are traced SEPARATELY (and the real objective stays the per-iteration :Ladder:, not the loss). A diverging-to-inf or collapsing-to-0 loss flags a bug; otherwise loss shape is diagnostic only.
