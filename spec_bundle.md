...
- :Sparse-depth-log: is a TRAINING/LOG reduction on the DEPTH axis (user: "we don't necessarily need to train on all depths"): the spearhead search still runs FULL iterative deepening 1..MAX (1-3 full width per :Full-width-floor:), but the per-position value is RECORDED — as a training target and for observability — at only a SPARSE set of depths, never every depth: {1, 3, 5, 8, MAX}, adaptive to the actual MAX reached (11 → {1,3,5,8,11}; 10 → {1,3,5,8,10}; 9 → {1,3,5,9}; 8 → {1,3,5,8}). Rationale: the value-vs-depth curve is smooth, so a Fibonacci-ish sparse sample that ALWAYS includes the deepest (the TD-Leaf target) captures it at a fraction of the log size + write cost. AXIS DISCIPLINE: this drops redundant intermediate DEPTHS PER POSITION — it is ORTHOGONAL to :Material-coverage: (the POSITION axis; positions are NEVER dropped, that would re-open the coverage hole). The MAX-depth value is the PRIMARY critic target (`V_search`); the shallower sparse depths {1,3,5,8} are AUXILIARY MULTI-DEPTH TRAINING TARGETS (user-confirmed): the critic is trained to predict the value at EACH sampled depth — sparse DEEP SUPERVISION on the search-depth axis (the eligibility-trace idea applied to depth, not just game moves), giving more signal per position cheaply. The iterative-deepening loop already computes each depth's value; :Sparse-depth-log: only chooses WHICH to persist + regress. A DA2C training feature: the critic's loss sums the (V_net − V_search@d)² over the sparse depth set d ∈ {1,3,5,8,max}.
