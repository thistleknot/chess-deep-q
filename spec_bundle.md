...
- :NNUE-eval: is the fast leaf evaluator: board → White-absolute centipawns via a small net over sparse king-relative features. It exists to break the one contradiction the whole measurement thread exposed — accuracy ⊥ nodes/sec: it is accurate like a net yet µs-cheap like pst (a handful of small matmuls over a ~256-wide :Accumulator:), so it can be searched DEEP, the property the 9.6 ms/call conv net lacked. It replaces/augments :Prior-lineage: pst as the :Eval-wiring: leaf. Its value frame is the :Value-target-convention: (White-absolute centipawns), identical to `engine.pst_eval`, so it drops into `AlphaBetaEngine` unchanged. PURPOSE (the repo's intent is chess-through-RL, so this is stated up front): :NNUE-eval: is NOT a supervised destination — it is the µs-cheap eval that makes DEEP self-play (d3+) affordable, i.e. the throughput fix that lets expert iteration AMPLIFY. It is Stage 1 of the RL :Prior-lineage: (supervised bootstrap → self-play outcomes take over the loss via :NNUE-training:), the enabling step for B, not a replacement of it. MEASURED — v1 FALSIFIED (this iteration): the torch king-bucketed v1 failed :NNUE-kill-check: Stage 1 on BOTH axes — 281 µs/call = 8× pst so it LOST at equal wall-time (a torch forward cannot feed a per-node search; real NNUE is hand-rolled int SIMD), AND d2 808 / d3 −280 vs pst 1367/1672 so it LOST at equal depth (the eval itself is weaker on 66k labels). STAGE-2 RETEST (enriched data, `RL_FINDINGS.md` coda 4): retrained the same arch on 156k QUIET, in-distribution, soft-MultiPV, ~3×-augmented positions → d2 −585 (~782), NO improvement on v1's 808. Root cause is not just volume: quiet strong-play trajectories are material-BALANCED (only 8% |cp|>750), so the eval undervalues material (queen-up +380) and search exploits the hole. The `:Quiet-filter:` strips the material-imbalanced-quiet positions the eval needs — moving the wall requires massive VOLUME **and** material-imbalance COVERAGE (won endgames / odds positions), not cleaner midgames. More important, the founding premise is CORRECTED: the throughput wall is not "the net is too slow" — the conv/resnet net is slow at ONE position but FAST BATCHED, so the RL-enabling fix is NOT a µs per-node eval, it is BATCHING the net across parallel PUCT self-play games (see `self-play-leela.spec.md` :Search-profile: PRIMARY). This spec is PARKED (not deleted): a faster leaf eval remains a valid future lever for DEEP alpha-beta, but it is no longer the RL path — the batched-PUCT :Amplification-experiment: supersedes it as the current move.
...
- :Policy-move-ordering: is the consumer of the :Policy-head: inside the net-minimax profile (minimax does not otherwise read P): the policy distribution orders moves for search and selects the beam. This closes a dead-weight defect (the policy head would otherwise be unused in the primary profile) and makes cloning self-reinforcing — a better policy yields better ordering, hence deeper effective search, hence better cloning targets. MEASURED (`measure_policyorder.py`, `tower.pth` SF-distilled policy, eval=pst): the policy IS a good ordering signal — at `policy_plies=2` it cuts **17% of nodes** to depth 4. BUT the resnet call is ~9.6 ms, too slow to apply at depth within a time budget; **root-only** (`plies=0`, one call/move) is all that's affordable and cuts only ~7%, which does NOT convert to strength (vs SF@1320 @0.3s, 20g: order-OFF ~1467 vs order-ON ~1409, overlapping CIs = a wash). The classical ordering (TT/MVV-LVA/killers/history) is already sufficient; the weak net's marginal root improvement doesn't beat it and its call cost offsets the small node saving. NOT wired into the deployed agent — the `policy_fn` hook remains for a µs-FAST policy (linear/quantized), which is what's needed to apply the ordering at depth and realize the 17%. Same throughput wall: the ordering benefit lives at depth, the resnet is too slow to reach it.
...
- :Deployed-agent: is the shipped play agent, realized on the SOUND alpha-beta `engine.py` (NOT the sampled PUCT/beam profiles above): :Critic-leaf: (NNUE) + :Move-category-weights: ordering + :Full-width-floor: + :Phi-widening: + :Tree-reuse:, with :Teacher-confinement:. It realizes the same INTENT as the beam's :Progressive-narrowing:/:Phi-rotation:/:Search-window-reuse: but soundly — negamax recurses over ALL moves, so with the refutation exemption there is NO sample-optimism (the beam concepts above carry an acknowledged optimism bias; this does not).
- :Full-width-floor: is the guaranteed shallow base of the :Deployed-agent:: while ply < PHI_FULL_WIDTH_FLOOR (=3) EVERY legal move is searched (no narrowing), so the agent never prunes inside the tactical horizon reachable in 3 plies. Below the floor, :Phi-widening: applies.
- :Phi-widening: is forward-pruning in `engine._negamax`: beyond the :Full-width-floor:, only the top-φ(depth) moves by :Move-category-weights: ordering are searched, φ(depth) a `fib_schedule` width budget (wide near root → 1 deep). REFUTATION-PRESERVING: captures and checks (`board.is_capture`, `board.gives_check`) are ALWAYS searched regardless of the budget — so every tactical reply survives at every node, INCLUDING opponent (min) nodes; that is why alpha-beta is sound where the sampled beam is not. It extends the engine's existing LMR (which REDUCES late quiet moves) to DROP them past the budget, same forced-move exemption. Trade: positional-pruning risk on late quiet moves accepted to buy depth; tactical soundness retained. Flag-gated `phi_widen` — OFF for the teacher `evaluate()` labeling path (must stay full-width/sound), ON for the :Deployed-agent:.
- :Depth-lambda: is the SCALAR that replaces a hand-picked depth table (user: "a scalar rather than me finger-fudging static table depths… a mini eligibility trace with a param to control TD vs Monte-Carlo depth"). Iterative deepening already computes `V_search(d)` at EVERY depth d=1..MAX for a position; the critic target is their GEOMETRIC ELIGIBILITY BLEND `target = Σ_d w(d)·V_search(d)`, `w(d) ∝ λ_depth^(MAX−d)` (normalized) — a single tunable knob, NOT a static set {1,3,5,8,max}. It is a TD↔MC dial ON THE SEARCH-DEPTH AXIS: λ_depth→0 weights the SHALLOW value (small effective-n, TD-like: biased, low-variance); λ_depth→1 weights the DEEP/MAX value (large n, MC-like: low-bias, high-variance). "n = depth" — the scalar sets the effective search-depth-n the target trusts, so we USE every depth's value (weighted), never skip-and-retrain a table. Purpose: variance-reduction on the per-position TD-Leaf target (the max-depth value can be noisy from frontier eval holes; the shallower values are more stable). DISTINCT AXIS from :Lambda-return: (`λ_game`≈0.85, the eligibility trace over real GAME MOVES): λ_depth builds a lower-variance per-position target from the depth curve, λ_game chains those targets along the game — same mechanism, orthogonal axes, they compose. Tunable exactly like λ_game/DA2C_CLC (literature-grounded band → Optuna, [[hyperparam-tuning-method]]). Honest: the right ABSTRACTION (one scalar, principled) — whether λ_depth beats simply using V(MAX) is empirical, decided by the tune. A DA2C critic-target feature (`DA2C_DEPTH_LAM`), off by default (V(MAX) only) until measured.
- :Tree-reuse: is the SOUND realization of :Search-window-reuse:: the :Deployed-agent: reuses ONE persistent `AlphaBetaEngine` across moves, so its Zobrist-keyed (position-absolute) transposition table carries every scored node forward — each move reprobes prior computation instead of recomputing. Requires depth-preferred REPLACEMENT (never clobber a deeper entry with a shallower one) + size-bounded EVICTION, and φ-pruned nodes stored as LOWER/UPPER bounds, NEVER EXACT (a forward-pruned node is not a true bound). Unlike the leaky beam reuse (subtree survives only under the actual reply), TT reuse is transposition-complete.
- :Critic-leaf: is the :Deployed-agent: leaf evaluator: the off-policy-distilled NNUE (`nnue_model.make_nnue_eval`), a `board → White-absolute cp` eval_fn with the exact `pst_eval` contract, plugged as `AlphaBetaEngine(eval_fn=...)`. It IS the :Learned-value: critic the project distills; the search reconstructs :Afterstate-action-value:s from it. Recompute-per-leaf caps depth until the incremental :Accumulator: (nnue-eval Stage 3) makes it µs — but phi-widened alpha-beta already deepens affordably at a time budget.
- :Teacher-confinement: is the boundary rule: STOCKFISH is teacher/data/measurement ONLY — never in the :Deployed-agent:'s move choice (confirmed absent from every play-path module); an agent that consulted the teacher at play time would not be an agent. The linear/PCA/gbdt eval is likewise NOT in the play path — the deployed critic is the NNUE alone. This confines the ε-teacher-vs-self exploration idea to DATA GENERATION (where it is :Trajectory-sampling: teacher-guidance), not the play-time search.
...
- The :Deployed-agent: lives in `engine.py` (the sound alpha-beta), NOT `mcts.py`/`beam.py` (the sampled profiles). Constants: PHI_FULL_WIDTH_FLOOR (=3) and the `phi_width(depth)` budget (reuse `beam.fib_schedule`). :Phi-widening: is `phi_widen`-gated (default OFF → teacher `evaluate()` stays full-width/sound; the agent sets it ON). It is wired via a `"nnue"` branch in `agents.make_agent` (one persistent engine → :Tree-reuse:) and registered in `menu._pick_agent`.
- :Tree-reuse: replacement lives at the TT store sites: depth-preferred (keep the deeper of colliding entries), size-capped eviction, and φ-pruned nodes flagged as bounds not EXACT.
...
- A :Phi-widening: soundness test: on tactical/refutation positions (the trap below, the free-queen position), `phi_widen=True` at a given nominal depth returns the SAME best move as full-width (`phi_widen=False`) — because captures and checks are exempt from the width budget, no refutation is pruned. Asserts the refutation-preservation that distinguishes the sound alpha-beta realization from the sampled beam.
...
================================================================================
FILE: spec\linear-value-rl.spec.md
================================================================================
...
- :Two-dial-hybrid: reframes the design as a POINT IN (τ, λ) SPACE, not a binary — both knobs were only ever run at their measured-broken extremes. **τ** (search/exploration breadth): self-play plays a temperature-softmax sample over the TOP-K root moves BY THE ENGINE'S ORDERING (`engine._order`), each scored by a shallow sound search — "scoring temperature near max moves." Because ordering ranks captures/checks/policy-moves high, the sample keeps refutations (unlike the uniform old beam that dropped them), so it stays sound while giving learning-exploration variety; τ→0 = argmax (no exploration), higher τ = wider. **λ** (value-target bias-variance): the target is `value_targets.lambda_return` (lam=0 → TD0, lam=1 → MC), bootstrapped from the per-state SOUND-SEARCH value — the tuned interior between the two failed endpoints (MC λ=1 → high-variance/overfit ~1140; TD0 λ=0 → low-Bellman-residual/worse-play ~659). The hybrid runs on the sound move-ordered alpha-beta with the linear eval and sweeps λ to find whether an interior (τ, λ) beats the endpoints. Honest ceiling: still 160-game linear — this tunes the interior, it does not break the data wall. Env-flag modes: `LINEAR_TARGET=lambda`+`LINEAR_LAMBDA`, `LINEAR_TAU`+`LINEAR_TEMP_PLIES`. MEASURED (τ=0.5, material+PST, λ sweep vs pst @d2): a clean INTERIOR PEAK — λ=0.0 −458, λ=0.5 −241, **λ=0.9 −176 (~1191, best)**, λ=1.0 −241. The interior (λ≈0.9) BEATS both endpoints (TD0 and MC) and the prior material+PST-MC best (~1144) — the bias-variance middle was real, confirming "it's a λ problem." First linear config to improve on plain MC. CAVEATS: small margins (n=30, draw-heavy; the ‸ SHAPE across four points is the signal, not the exact peak), and still 0/30 wins vs pst @d2 (closer, not through — the ~50 Elo gain doesn't break the linear/data wall). Best linear agent to date = the two-dial hybrid (τ≈0.5, λ≈0.9). OPTUNA-TPE FOLLOW-UP (disciplined redo per [[hyperparam-tuning-method]]: literature-grounded λ∈[0.70,0.97]×τ∈[0.2,0.8], TPE, n=60, `tune_lambda.py`, SQLite-persisted): best λ=0.848 τ=0.428 (score 0.380) — λ lands in the literature 0.83–0.85 band (and near Giraffe's TD-Leaf λ=0.7). BUT the objective is NOISE-DOMINATED (near-identical (λ,τ) gave 0.07 vs 0.24) — read as "high-λ ≈0.85 ± noise," not a razor-resolved optimum. LESSON: don't tune what you can't resolve; λ≈0.85 is settled, the remaining lever is DATA (Stage 1), not the knob.
...
- :Critique-vs-behavior: is the load-bearing taxonomy the RL family is grounded in: the BEHAVIOR policy (how self-play picks/EXPLORES moves — the :Spearhead-profile: + temperature/Dirichlet) and the CRITIQUE policy (whose next-action value the TARGET bootstraps from) are INDEPENDENT axes. When they DIFFER the method is OFF-policy; when they are the SAME it is ON-policy. Q-learning behaves ε-greedy/exploratory but its target is `max` (the GREEDY critique) — that split IS off-policy. Put ε-greedy INTO the target and you have renamed the algorithm: sampled → SARSA, expectation → Expected SARSA. Exploration never enters the critique; that is the whole off-policy trick (behave exploratorily, JUDGE as if optimal from s' onward).
- :Critic-criterion: is the definition of a critic: it BOOTSTRAPS (a prediction from a prediction, S&B). A Monte-Carlo baseline is NOT a critic — REINFORCE (target Gₜ) has no value function; +baseline subtracts V OUTSIDE the return (variance reduction only, still MC); the critic is BORN only at the TD/bootstrap step. So the progression is REINFORCE (nothing) → +baseline (still no critic) → +bootstrap (critic). This grounds REINFORCE as the no-critic FLOOR of the benchmark.
- :RL-family-benchmark: is the four-way comparison — REINFORCE, SARSA, Q-learning, :Advantage-actor-critic: — all TD-Leaf(λ) variants on the SAME :Spearhead-profile: substrate + shared afterstate-value net, differing ONLY in the per-update :Update-rule: (the `da2c.gradient_step` seam). All bootstrap from `V_search` (the spearhead minimax backup = TD-Leaf); λ is the shared eligibility-trace dial (≈0.85). The four span the classic axes: value/policy × on/off-policy × MC/TD(λ). Each :Update-rule::
  - :REINFORCE-update: target = Gₜ (`value_targets.mc_return`, = λ-return at λ=1); NO value in the target; policy-gradient only. The no-critic floor (≈ the already-plateaued outcome self-play, now on the SOUND operator).
  - :SARSA-update: target = r + γ·`V_search`(TAKEN successor afterstate); on-policy — critiques the exploring policy itself (the taken move's value, exploration included). Realized as `lambda_return(λ→0)` over the CHOSEN-move backed-up values.
  - :Q-learning-update: target = r + γ·MAX over afterstate `V_search` (the spearhead minimax IS the greedy critique); OFF-policy → ingests the ~1672-engine games (:Off-policy-handoff:) via a REPLAY buffer + hard-synced TARGET NET (the deadly-triad stabilizers: FA + bootstrap + off-policy). Reuses `legacy.DQNAgent.memory`/`update_target_network`.
  - :A2C-update: advantage = `V_search − V_net`, actor (afterstate-softmax) + bootstrapping critic, :Lambda-return: (= :Advantage-actor-critic:, TD-Leaf(λ)).
  Critique-policy table (documented for grounding): REINFORCE → no value / no critique-policy / no critic; SARSA → V in target / ε-greedy behavior critique / true critic; Q-learning → V in target / GREEDY critique regardless of behavior / true critic; A2C → V in target / current-policy critique / true critic. Honest disposition on the axes: A2C > REINFORCE (definitional — adds the critic); A2C vs Q-learning is a TRADE (A2C on-policy-stable + explicit actor; Q-learning off-policy sample-efficient + reuses scarce/teacher data) — and since search RECONSTRUCTS the policy from the value, the actor is partly redundant, so value-based Q-learning/TD-Leaf is the natural chess fit (Giraffe's lineage). DEFERRED (decision): the benchmark is kept as an available LANE (spec'd, `da2c.gradient_step` is the seam), NOT built now — because the disposition already identifies the actor-critic/TD-Leaf path as the fit, the critical path to the >1600 GOAL is continuing to TRAIN that path (the distillation climb + DA2C self-improve + the :Accumulator: speed unlock), not comparing priors. Build the other three arms only if training stalls and the prior choice becomes the suspected bottleneck.
- :Frame-discipline: is the mandatory sign convention for the actor-critic arms (da2c-verified defect): the CRITIC value + its :Lambda-return: target stay WHITE-ABSOLUTE (the `value_targets` convention, single source of truth `to_white`/`to_stm`), but the ACTOR's afterstate softmax AND its advantage MUST be converted to the SIDE-TO-MOVE frame before the policy-gradient step — else Black's strong moves (which LOWER the White-absolute value) receive low probability and a negative advantage, and the gradient pushes them the WRONG way.
...
- Constant (:DA2C:): DA2C_WORKERS (parallel self-play processes), DA2C_NSTEP (bootstrap horizon), DA2C_LR, DA2C_CLC. **DA2C_CLC is a TUNABLE hyperparameter, not a fixed 0.1** — it is the actor/critic learning-rate BALANCE (down-weights the critic loss so the actor learns faster), tuned by the same disciplined method as λ/τ ([[hyperparam-tuning-method]]: literature-grounded band ~0.05–0.5, Optuna-TPE, noise-adequate n) once the loop runs. File: NEW `da2c.py` — shared-memory dual-head critic + N `torch.multiprocessing` workers running :Spearhead-profile: self-play + the advantage-actor-critic gradient step; reuses `engine.AlphaBetaEngine(phi_widen=True)`, the critic net, `value_targets.lambda_return`, `measure_ladder`.
- :Loss-trace: — the loop's training observability: a persistent trend log + a MULTI-PANEL plot of FOUR metrics over training time — (1) ACTOR loss, (2) CRITIC loss, (3) MEAN GAME LENGTH in turns (per self-play game; ch5 fig 5.14/5.15), (4) MEAN ELO (the periodic :Ladder: result, the real objective). Actor and critic are adversarial (ch5 fig 5.13) so their combined loss is chaotic BY DESIGN — they are traced SEPARATELY, and the losses are DIAGNOSTIC ONLY (diverging-to-inf/collapsing-to-0 flags a bug); the OBJECTIVE that decides success is mean Elo (rising = amplification), with game length as a secondary progress signal. Traced separately, plotted together (four panels).
- The :Amplification-experiment: verdict (rising per-iteration :Ladder:) applies unchanged — DA2C is viable here iff the spearhead-operator self-play makes the ladder RISE.
